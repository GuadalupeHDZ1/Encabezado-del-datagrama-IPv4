...
class NumberValidator {
    ...
    static validateHexadecimal(input, bits) {
        if (!input) return { valid: false, message: 'Campo requerido' };
        if (!/^[0-9A-Fa-f]+$/.test(input)) return { valid: false, message: 'Solo hexadecimal (0-9, A-F)' };
        
        const maxDigits = Math.ceil(bits / 4);
        if (input.length > maxDigits) return { valid: false, message: `Máximo ${maxDigits} dígitos` };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('options_hex').addEventListener('input', function() {
        updateOptionsInfo();
        updateTotalLengthPreview();
    });
...
function updateTotalLengthPreview() {
    ...
    // --- Cálculo del encabezado ---
    // IHL incluye TODOS los bytes del encabezado  
    // PERO tú quieres sumar explícitamente los 4 bytes iniciales
    // Entonces:
    // 4 bytes (versión, ihl, tos, totalLength) + (ihl*4 - 4 restantes)
    const headerBytes = (ihl * 4);  // valor real del estándar (20 si ihl=5)
    const headerBytesWithVersion = headerBytes + 4; // tu forma personalizada
